...
const AddSubscription = (props) => { 
    ...
    React.useEffect(() => {
        if(props.subscriptions) {
            if(query.get("subscriptionId")) {
                props.subscriptions.forEach(sub => {
                    if(sub._id == query.get("subscriptionId")) { 
                        setFormData({...formData,...sub,subscriptionId: sub._id})
                    }
                })
            } else {
                props.showAlert("Data Not Found")
                props.history.push("/admin/subscriptions/VIEW-SUBSCRIPTION")
            }
        } else {
    ...
    const onSubmit = () => {
        if(validate()) {
            ...
            axios({
                method: "put",
                url: "/subscription/editSubscription",
                data: {
                    ...formData
                }
            }).then(res => {
                setLoading(false);
                props.showAlert("Subscription Updated Succesfully");
                props.getAllSubscriptions()
    ...
    return (
        <div className={styles.container}>
            <Paper variant="outlined" className={styles.paper}>
                <h1>{LANG.EDIT} {LANG.SUBSCRIPTION}</h1>
                ...
                <div className={styles.row}>
                    ...
                    <Button color="primary" variant="contained" startIcon={<CircularProgress color="inherit" size={20} />}>{LANG.LOADING}</Button>
                        :
                    <Button color="primary" variant="contained" startIcon={<UpdateRoundedIcon />} onClick={onSubmit}>{LANG.UPDATE} {LANG.SUBSCRIPTION}</Button>}
                </div>
            </Paper>
        </div>
    )
...
export default withRouter(connect(mapStateToProps,{showAlert,getAllSubscriptions})(AddSubscription));