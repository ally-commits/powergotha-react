...
const EditDelivery = (props) => { 
            ...
            if(query.get("userId")) {
                props.users.forEach(user => {
                    if(user._id == query.get("userId")) {
                        let arr = user.assignedWarehouse.map(val => val._id)
                        setFormData({...formData,...user,userId: user._id,assignedWarehouse: [...arr],dob: user.dob.substring(0,10)})
                    }
                })
            } else {
    ...
    const onSubmit = () => {
        if(validate()) {
            ...
            axios({
                method: "put",
                url: "/dashboard/delivery/editUser",
                data: {
                    ...formData
                }
            }).then(res => {
...
export default withRouter(connect(mapStateToProps,{showAlert,getAllWarehouse,getAllUsers})(EditDelivery));